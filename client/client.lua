...
RegisterNetEvent('smdx-bathing:client:StartBath')
AddEventHandler('smdx-bathing:client:StartBath', function(town)
    inbath = true 
    if Config.BathingZones[town] then
        ...
        TriggerEvent("smdx-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { cache.ped, "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@ARTHUR`, `DEFAULT`, "BATHING" })
        TriggerEvent("smdx-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { rag, "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@RAG`, `DEFAULT`, "BATHING" })    
...
ExitBathing = function(animscene, town, cam)
    ...
    TriggerServerEvent("SMDXCore:Server:SetMetaData", "cleanliness", 100)
    TriggerServerEvent("smdx-bathing:server:setBathAsFree", town)
...
RegisterNetEvent('smdx-bathing:client:StartDeluxeBath')
AddEventHandler('smdx-bathing:client:StartDeluxeBath', function(animscene, town, cam)
    if not IsPedMale(cache.ped) then
        ...
        TriggerEvent("smdx-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { cache.ped, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@ARTHUR`, `DEFAULT`, "BATHING" })
        TriggerEvent("smdx-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { BathingPed, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@MAID`, `DEFAULT`, "BATHING" })    
...
RegisterNetEvent('smdx-bathing:client:HideDeluxePrompt')
AddEventHandler('smdx-bathing:client:HideDeluxePrompt', function()
...
ExitPremiumBath = function(animscene, town, cam, disableScrub)
    ...
    TriggerEvent("smdx-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { cache.ped, "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@ARTHUR`, `DEFAULT`, "BATHING" })
    TriggerEvent("smdx-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { BathingPed, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@REGULAR@MAID`, `DEFAULT`, "BATHING" })    
...
UndressCharacter = function()
    ...
    EquippedWeapons = exports['smdx-weapons']:EquippedWeapons()

    for i = 1, #EquippedWeapons do
        RemoveWeaponFromPed(ped, EquippedWeapons[i])
    end
    TriggerEvent('smdx-wardrobe:client:removeAllClothing')
end
...
Action = function(name, p1, p2, p3)
    ...
    if (name == "START_BATHING") then
        TriggerServerEvent("smdx-bathing:server:canEnterBath", p1)
    elseif (name == "REQUEST_DELUXE_BATHING") then
        TriggerServerEvent("smdx-bathing:server:canEnterDeluxeBath", p1 , p2 , p3)
